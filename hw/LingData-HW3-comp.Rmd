...
Число значений, которые превышают 5 равно ¼. Доля не превышающих значений состоит из 3 столбцов с показателями частоты 1, 10 и 4 (в сумме — 15), Доля значений выше 5 — из двух столбцов с показателями 4 и 1 (в сумме — 5). Общее число — 20. 5 из 20 — это ¼.
b. Indicate the interval where the median of this sample can lie. Explain your answer.
Медиана будет на промежутке от -5 до 0. Так как всего Х-ов — 20, нас интересует полусумма 10 и 11 при расположении от меньшего Х к большему. Оба они будут на этом промежутке.
c. How the histogram will change if we add an element $7$ to the sample? Explain your answer.
Четвёртый столбец, изображающий промежуток от 5 до 10 увеличится на одно деление и по оси «Частота» будет равен пяти (т.к. 7 входит в промежуток от 5 до 10).
...
Во втором случае дисперсия будет выше, так как в первом случае график-купол, описывающий первую диаграмму, был бы более плавным, чем во втором случае, так как во втором случае разброс будет сильнее.
\newpage
...
Which of the samples has a larger variance? Explain your answer.
В первом случае дисперсия будет выше, так как здесь больше столбцов и выше разброс, поэтому график-купол, описывающий эту диаграмму, будет более резко подниматься и опускаться, чем во втором случае.
## Part 2
...
Слева от 35 гораздо больше значений, которые выше по своей частоте, справа же — небольшой хвост с преимущественно небольшими значениями.
...
```
Explain your answer below:
Большая часть столбцов (в том числе — с самой высокой частотой)находятся справа от значения 10%, а значит, большинство учеников допустили больше 10 ошибок.
...
```
Explain your answer below:
Число 20 находится ближе остальных к вершине "купола", при этом в обе стороны от вершины количество ошибок уменьшается довольно плавно, но справа много значений, которые встречаются сильно реже.
...
```
Нет, нулевую гипотезу отвергнуть нельзя.
```
Explain your answer below:
P-value будет больше 0,05: степень свободы равна 1, хи-квадрат равно 1,000001 ((999999-1000000)^2/1000000+(0-1)^2/1), следовательно, согласно таблице, p-value будет в диапазоне от 0,2 до 0,975, поэтому нулевую гипотезу отклонить нельзя.
...
```
H0: Глаголы в рифмующейся позиции в 1920-х употребляются так же часто, как и глаголы в общем (p=17%).
H1: Глаголы в рифмующейся позиции в 1920-х чаще или реже, чем глаголы в общем (p>17% | p<17%).
```
...
library(tidyverse)
poems <- read.csv('https://raw.githubusercontent.com/LingData2019/LingData2020/master/data/poetry_last_in_lines.csv', sep = '\t')
poems %>%
  filter(Decade == '1920s') %>%
  nrow()
poems %>%
...
```
P-value получился больше, чем 0,05 (0,2297), а значит, мы не можем отвергнуть нулевую теорию. На сэмпле из 1920-х годов p=0,141, что немного реже чем мы предполагали.
```
...
```
P-value получился больше, чем 0,05 (0,8873), а значит, мы не можем отвергнуть нулевую теорию. На сэмпле из 1820-х годов p=0,172, что при округлении до сотых соответствует тому, что мы и предполагали.
Если в целом, то в 20-х годах 19 века глагольной рифмой грешили больше, чем в 20-х годах века 20 :)
```
...
```
H1: Средняя продолжительность гласного у "shg05" больше или меньше 73 мс (p > 73 ms | p < 73 ms).
```

### 8.2
Perform a one-sample t-test. 
```{r}
Icelandic <- read.csv('https://raw.githubusercontent.com/LingData2019/LingData2020/master/data/icelandic.csv', sep=',')
t.test(Icelandic[Icelandic$speaker == 'shg05',]$vowel.dur, mu=73)
```

### 8.3
Interpret results. 
```
P-value немного больше, чем 0,05 (0,05172), поэтому мы снова не можем отвергнуть нулевую теорию.
```
