...
count(filter(mono_socio, Responce == 'female'))
count(filter(mono_socio, Responce == 'male'))
education <- mono_socio$Responce[mono_socio$QuestionCategory == 'education']
unique(education)
count(filter(mono_socio, Responce == 'College'))
count(filter(mono_socio, Responce == 'UniversityDegree'))
count(filter(mono_socio, Responce == 'Postgraduate'))
count(filter(mono_socio, Responce == 'PhDongoing'))
count(filter(mono_socio, Responce == 'right'))
count(filter(mono_socio, Responce == 'left'))
```
...
```{r}
library(ggplot2)
ggplot(data = mono, aes(x = RT)) + geom_density()
```
...
Это не похоже на нормальное распределение, т.к. у графика очень сильный перекос в левую сторону.
...
```{r}
ggplot(data = mono, aes(x = RTlog)) + geom_density()
```
...
Такой график гораздо больше похож на нормально распределение. У него нет больших перекосов, и у него не такой резкий купол, как у предыдущего графика.
...
```{r}
mono1 <- mono %>% filter(RT > 600 & abs(RTlog - mean(mono$RTlog)) <= 3 * sd(mono$RTlog) & TypeOfStimuli != "FillerAcceptable" & TypeOfStimuli != "FillerUnacceptable") %>% select(ParticipantID, TypeOfStimuli, WordOrder, AcceptabilityJ = ResponseAcceptabilityJudgement, RT, RTlog) %>% mutate(ParticipantID = as.factor(ParticipantID), TypeOfStimuli = as.factor(TypeOfStimuli), WordOrder = as.factor(WordOrder), AcceptabilityJ = as.factor(AcceptabilityJ)) 
...
```{r}
summa <- mono1 %>% group_by(TypeOfStimuli, AcceptabilityJ, WordOrder) %>% summarise(number = n())
AJ <- summa$AcceptabilityJ
ggplot(data = summa, aes(x = TypeOfStimuli, y = number, fill = AJ)) + geom_col(colour = 'black', position = "dodge") + facet_wrap(vars(WordOrder))
```
...
```{r}
summa2 <- mono1 %>% group_by(AcceptabilityJ, WordOrder) %>% summarise(number = n())
AJ <- summa2$AcceptabilityJ
ggplot(data = summa2, aes(x = WordOrder, y = number, fill = AJ)) + geom_col(colour = 'black', position = "dodge")
```
...
```{r}
Order <- mono1$WordOrder
ggplot(mono1, aes(x = AcceptabilityJ, y = RTlog, fill = Order)) +   geom_violin(position = position_dodge(1)) + geom_boxplot(width=0.35, position = position_dodge(1))
```
...
#library(ggpubr)
ggline(mono1, x = "AcceptabilityJ", y = "RTlog", add = "mean_se")
```
...
```{r}
mean_rtlog_long <- mono1 %>% group_by(ParticipantID, WordOrder) %>% summarise(mean = round(mean(RTlog), 2))
mean_rtlog_long
```
### 2.2. Pivoting
...
Нет, мы не можем отвергнуть нулевую гипотезу, т.к. p-value > 5%.

What claim about logarithms of reaction time for Congruent and Incongruent stimuli can you make according to this test?

Логарифмы, относящиеся к Incongruent stimuli, судя по средним значениям, незначительно выше.
...
```{r}
mean_rtlog <- mean_rtlog %>% mutate(diff = Incongruent - Congruent)
t.test(mean_rtlog$diff, mu = 0, alternative = "two.sided")
```

Whould you reject null hypothesis?

Да, так как p-value > 5%.

What claim about logarithms of reaction time for Congruent and Incongruent stimuli can you make now?

Отвергая нулевую теорию мы утверждаем, что в этих двух случаяхвремя реакции различно и независимо друг от друга.

How can you interpret difference with the result of 2.3?

В первом случае мы сравнивали всего два значения, которые немного отличались друг от друга, а во втором — много значений, который отличались друг от друга где-то чуть больше, а где-то — немного.
...
```{r}
incong_rtlog <- mono1 %>% filter(WordOrder == "Incongruent") %>% group_by(ParticipantID, TypeOfStimuli) %>% summarise(RTlog = round(mean(RTlog), 2))
incong_rtlog
```
...
```{r}
var_aov <- aov(RTlog ~ TypeOfStimuli, incong_rtlog)
summary(var_aov)
#anova используется для определения того, отличаются ли средства двух или более групп друг от друга. Здесь как раз такой случай: RTLog — независимая переменная, а TypeOfStimuli - факторная (категориальная) переменная, представляющая членство в группе.
```

F-значение меньше 5%, что означает, что мы отвергаем нулевую теорию, что, в свою очередь, подтверждает наличие существенной разницы среди результатов при разных условиях.
...
Значимая разница у пары SubjectiveComment-Material-Shape-Color (p<5%).
